...
for i in range(1, len(table_rows)):
    ...
    values = {
        ...
        'build': clean_text(tds[3].text),
        'images': 'https://' + tds[4].find('img').get('src').split("//")[1] if tds[5].find('img') else "NO_IMAGE",
    }
    data.append(values)
...
stadium_df.to_csv('data/stadium_clean2.csv', index=False)
...
with open('world_cup_players.csv', 'w', newline='', encoding='utf-8') as csvfile:
    csv_writer = csv.writer(csvfile)
    # Write header row
    csv_writer.writerow(['Group', 'Number', 'Position', 'Player Name', 'Age', 'Club'])
    ...
    for group in groups:
        ...
        # Check if the table exists for the group
        if table:
            group_name = group.split('_')[-1]  # Get the group name
            ...
            for row in table.find_all('tr')[1:]:
                ...
                csv_writer.writerow([group_name, number, position, player_name, age, club])

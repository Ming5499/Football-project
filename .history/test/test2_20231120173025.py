...
countries = table.find_all('table', {'class': 'sortable'})

for country in countries:
    # Extract country name
    country_name = country.find_previous('h3').find('span', {'class': 'mw-headline'}).text.strip()
    print(f"Country: {country_name}")

    # Extract player data for the country
    players = country.find_all('tr', {'class': 'nat-fs-player'})
    ...
    for player in players:
        ...
        if len(columns) > 1:
            ...
            # Print the extracted data (you can process or store

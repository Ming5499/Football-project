...
for group in groups:
    ...
    # Extract country name from the table header row for each player
    for row in table.find_all('tr')[1:]:
        country_name = None

        # Check if the row contains the target element
        if row.find('span', class_='mw-headline'):
            country_name = row.find('span', class_='mw-headline').text.strip()

        # Process country name if it exists
        if country_name:
